...
sidebar_position: 2
title: Sign In
description: Secure user authentication flow using Keycloak IAM with OAuth2 Authorization Code and PKCE
keywords: [sign in, authentication, Keycloak, OAuth2, PKCE, identity management, SSO, authorization code flow]
last_update:
  author: Nhan Nguyen
---
...
sequenceDiagram
    ...
    Client->>Keycloak: Authorization Request<br/>response_type=code<br/>client_id=edvantix-client<br/>redirect_uri=callback<br/>code_challenge=<hash><br/>code_challenge_method=S256<br/>scope=openid profile
    ...
    Client->>Keycloak: Token Request<br/>grant_type=authorization_code<br/>code=<auth_code><br/>client_id=edvantix-client<br/>code_verifier=<original_verifier><br/>redirect_uri=callback
