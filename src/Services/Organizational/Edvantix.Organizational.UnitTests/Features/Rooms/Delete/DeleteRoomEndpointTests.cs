...
public sealed class DeleteRoomEndpointTests
{
    private readonly DeleteRoomEndpoint _endpoint = new();
    ...
    public async Task GivenValidId_WhenHandling_ThenShouldReturnNoContent()
    {
        ...
        _senderMock
            .Setup(s => s.Send(new DeleteRoomCommand(id), It.IsAny<CancellationToken>()))
    ...
    public async Task GivenValidId_WhenHandling_ThenShouldCallSenderOnce()
    {
        ...
        _senderMock.Verify(
            s => s.Send(new DeleteRoomCommand(id), It.IsAny<CancellationToken>()),
            Times.Once
        );
    }
}
