...
description: Implementation of API Gateway pattern using YARP for routing and cross-cutting concerns
keywords: [ADR-008, API gateway, YARP, reverse proxy, routing, load balancing, rate limiting]
last_update:
  author: Nhan Nguyen
---
...
Edvantix's microservices architecture consists of multiple services with different endpoints, authentication requirements, and communication protocols. Client applications need a unified way to interact with the distributed system while addressing several challenges:
...
Implement an **API Gateway pattern** using **YARP (Yet Another Reverse Proxy)** integrated with **Aspire** for service discovery and routing, providing a unified entry point for all Edvantix microservices.
...
Edvantix uses a fluent builder pattern to configure service routing:
...
4. **Service Discovery**: Seamless integration with Aspire service discovery
...
Edvantix's API Gateway uses a simple, effective approach:
        ...
        .WithConfiguration(yarpBuilder =>
        {
            foreach (var service in services)
            {
                ...
                routeBuilder
                    ...
                    .WithTransformResponseHeader("X-Powered-By", "Edvantix Gateway");
            }
...
- **Development Experience**: Seamless integration with Aspire development workflow
...
- **Dependencies**: Tight coupling with Aspire orchestration platform

### Risk Mitigation

- **Aspire Integration**: Leverages Aspire's built-in reliability features
...
- ✅ YARP-based reverse proxy with Aspire integration
