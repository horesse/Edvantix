...
```bash
TRANSITION=$(node "E:/projects/Edvantix/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "E:/projects/Edvantix/.claude/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "E:/projects/Edvantix/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
