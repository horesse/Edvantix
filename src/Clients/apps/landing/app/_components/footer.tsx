      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-16 grid grid-cols-2 md:grid-cols-6 gap-8 lg:gap-12">
          ...
          <div className="col-span-2">
            <Link
              href="/"
              className="flex items-center gap-2.5 mb-4 w-fit group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-md"
              aria-label="Edvantix — на главную"
            >
              <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center shadow-lg shadow-primary/20 group-hover:shadow-primary/35 transition-shadow">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed mb-6 max-w-[220px]">
              Платформа управления онлайн-школой нового поколения. Всё в одном месте.
            </p>
            ...
            <div className="flex gap-2" role="list" aria-label="Социальные сети">
              {SOCIAL_LINKS.map((social) => (
                <a
                  ...
                  role="listitem"
                  className="w-9 h-9 rounded-lg bg-muted/50 border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted hover:border-border transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
                  {social.icon}
                </a>
              ))}
          ...
          {FOOTER_LINKS.map((group) => (
            <nav key={group.heading} className="col-span-1" aria-label={group.heading}>
              <h3 className="text-foreground font-semibold text-sm mb-4">{group.heading}</h3>
              <ul className="flex flex-col gap-2.5" role="list">
                {group.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-muted-foreground hover:text-foreground text-sm transition-colors duration-200 flex items-center gap-1 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded"
                    >
                      {link.label}
