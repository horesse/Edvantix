...
public sealed class Subscription() : Entity<long>, IAggregateRoot
{
    ...
    public Subscription(string name, SubscriptionType type, decimal price, long currencyId)
        : this()
    {
    ...
    public void UpdateName(string newName)
    {
        if (string.IsNullOrWhiteSpace(newName))
            throw new ArgumentException(
                "Название подписки не должно быть пустым.",
                nameof(newName)
            );
    ...
    public void AddLimit(LimitType type, decimal value)
    {
        if (_limits.Any(l => l.Type == type))
            throw new InvalidOperationException(
                $"Ограничение вида {type} уже существует для данного вида подписки."
            );
    ...
    public void RemoveLimit(LimitType type)
    {
        ...
        if (limit == null)
            throw new InvalidOperationException(
                $"Ограничение вида {type} не существует для данного вида подписки."
            );
