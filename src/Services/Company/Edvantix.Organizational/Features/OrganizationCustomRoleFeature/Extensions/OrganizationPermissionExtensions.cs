...
public static class OrganizationPermissionExtensions
{
    ...
    )
    {
        return builder.AddEndpointFilter(async (context, next) =>
        {
            ...
            var permService = context.HttpContext.RequestServices.GetRequiredService<IOrganizationPermissionService>();
            var hasPermission = await permService.HasPermissionAsync(profileId.Value, organizationId, permission);

            if (!hasPermission)
            {
                return Results.Json(
                    new { Detail = $"У вас нет разрешения '{permission}' в данной организации." },
                    statusCode: StatusCodes.Status403Forbidden
                );
            }
