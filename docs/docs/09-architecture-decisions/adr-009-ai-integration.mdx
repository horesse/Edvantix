...
description: AI integration strategy with Agent Framework for conversational support and recommendations
keywords: [ADR-009, AI integration, Agent Framework, LLM, conversational AI, semantic search, recommendations]
last_update:
  author: Nhan Nguyen
---
...
Edvantix aims to enhance user experience through intelligent features that leverage artificial intelligence capabilities. The system requires AI integration for several key scenarios:
...
Implement a **comprehensive AI integration strategy** using **Ollama** for local LLM hosting, **Agent Framework** for orchestration, **Model Context Protocol (MCP)** for standardized tooling, and **multi-agent workflows** for complex AI operations.
...
- **Agent Framework**: AI orchestration and workflow management
...
graph TB
    subgraph "AI Infrastructure Layer"
        ...
        AgentFramework[Agent Framework<br/>AI Orchestration]
    ...
    AgentFramework --> Ollama
    AgentFramework --> Qdrant
    AgentFramework --> MCPTools

    ChatService --> AgentFramework
    SearchService --> Qdrant
    RecommendService --> AgentFramework
    ContentService --> AgentFramework
...
COPY ./models/Edvantix-fine-tuned.gguf /root/.ollama/models/
```
