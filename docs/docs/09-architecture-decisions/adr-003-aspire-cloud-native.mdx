---
sidebar_position: 4
title: ADR-003 - Aspire for Cloud-Native Development
description: Selection of Aspire for cloud-native orchestration and built-in observability
keywords: [ADR-003, Aspire, cloud-native, service orchestration, observability, telemetry]
last_update:
  author: Nhan Nguyen
---

# ADR-003: Aspire for Cloud-Native Development
...
Edvantix's microservices architecture requires sophisticated orchestration, service discovery, observability, and deployment capabilities. The development and operational challenges include:
...
Adopt **Aspire** as the cloud-native development framework to provide integrated service orchestration, observability, and deployment capabilities for the Edvantix microservices ecosystem.

### Core Aspire Components

#### App Host (`Edvantix.AppHost`)
...
#### Service Defaults (`Edvantix.ServiceDefaults`)
...
The `Edvantix.AppHost` project orchestrates the entire system topology:
...
var catalog = builder.AddProject<Projects.Edvantix_Catalog>("catalog")
    .WithReference(postgres)
    .WithReference(redis)
    .WithReference(qdrant);

var basket = builder.AddProject<Projects.Edvantix_Basket>("basket")
    .WithReference(redis)
    .WithReference(catalog);

var ordering = builder.AddProject<Projects.Edvantix_Ordering>("ordering")
    .WithReference(postgres)
...
The `Edvantix.ServiceDefaults` package provides:
...
1. **Startup**: `dotnet run --project Edvantix.AppHost` starts entire ecosystem
...
- **Framework Dependency**: Tight coupling to Aspire ecosystem
