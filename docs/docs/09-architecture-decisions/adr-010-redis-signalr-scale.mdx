...
description: Use of Redis backplane for SignalR horizontal scaling and distributed caching
keywords: [ADR-010, Redis, SignalR, horizontal scaling, backplane, distributed caching, session management]
last_update:
  author: Nhan Nguyen
---
...
Edvantix's microservices architecture requires high-performance caching and real-time communication scaling capabilities. The system faces several performance and scalability challenges:
...
Edvantix uses Redis primarily for distributed state management and real-time communication scaling, with services utilizing Redis based on their specific needs:
...
Edvantix implements a custom Redis backplane for the Chat service, providing:
...
public class SignalRConfiguration
{
        ...
        string redisConnectionString)
    {
        ...
        {
            options.Configuration.ChannelPrefix = "Edvantix.SignalR";
        });
