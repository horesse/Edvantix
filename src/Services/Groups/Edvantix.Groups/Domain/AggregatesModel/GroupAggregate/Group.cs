...
public sealed class Group() : Entity, IAggregateRoot, ISoftDelete, ITenanted
{
    ...
    /// <param name="courseId">Идентификатор курса из Curriculum-сервиса (логическая FK, cross-service).</param>
    /// <param name="teacherMemberId">Идентификатор преподавателя (логическая FK, cross-service).</param>
    /// <param name="format">Формат занятий.</param>
    /// <param name="roomId">Кабинет; обязателен при <c>Offline</c>/<c>Mixed</c>. Логическая FK (cross-service).</param>
    ...
    /// Логическая FK — без constraint на уровне БД (cross-service).
    /// </summary>
    public Guid CourseId { get; private set; }

    /// <summary>
    /// Идентификатор преподавателя группы.
    /// Логическая FK на OrganizationMember из Organizational-сервиса (cross-service).
    /// </summary>
    ...
    /// Логическая FK на Room из Organizational-сервиса (cross-service).
    ...
    /// <summary>Дата окончания работы группы. NOT NULL — группа всегда имеет плановую дату завершения.</summary>
    ...
    /// <summary>Фиксирует выход участника из группы.</summary>
    ...
    /// <summary>Восстанавливает архивированную группу, возвращая её в статус <see cref="GroupStatus.Recruiting"/>.</summary>
