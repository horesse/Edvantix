...
public sealed class UpdateRoomEndpointTests
{
    ...
    {
        var command = new UpdateRoomCommand(Guid.CreateVersion7(), "Каб. 204", Floor: 2, Seats: 20);
        _senderMock
    ...
    {
        var command = new UpdateRoomCommand(Guid.CreateVersion7(), "Зал А", Floor: 1, Seats: 50);
        _senderMock
            .Setup(s => s.Send(command, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Unit.Value);
