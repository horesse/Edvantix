...
Edvantix requires a cloud deployment platform that supports containerized microservices with auto-scaling and service discovery. The deployment needs:
...
Edvantix uses `azd` for streamlined Azure deployment:
