...
description: Edvantix's business partners, external entities, and domain-specific interactions with stakeholders
keywords: [business context, external partners, communication patterns, book data providers, email service, AI provider, identity provider]
last_update:
  author: Nhan Nguyen
---
...
The business context shows Edvantix's interactions with its business environment, focusing on the domain-specific inputs and outputs exchanged with external entities.

```mermaid
C4Context
    title System Context Diagram for Edvantix
    ...
    Boundary(b1, "Edvantix Organization", "Internal Systems") {
        System(Edvantix, "Edvantix System", "Digital bookstore platform providing book discovery, purchasing, and customer support")
    }
    ...
    Rel(customer, Edvantix, "Browses catalog, places orders, uses chat support", "HTTPS")
    Rel(admin, Edvantix, "Manages system, monitors operations", "HTTPS")

    Rel(Edvantix, bookDataProvider, "Retrieves book information", "REST API")
    Rel(Edvantix, emailService, "Sends notifications", "SMTP/API")
    Rel(Edvantix, aiProvider, "Gets AI responses", "REST API")
    Rel(Edvantix, identityProvider, "Authenticates users", "OIDC")
