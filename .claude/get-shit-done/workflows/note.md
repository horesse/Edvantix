...
- **Global scope**: `E:/projects/Edvantix/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/projects/Edvantix/.claude/notes/`)
...
2. Glob `E:/projects/Edvantix/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/projects/Edvantix/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/projects/Edvantix/.claude/notes/` — works in any directory
