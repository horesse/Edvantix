...
- [ ] **ORG-01**: Владелец школы может создать кастомную роль с именем в рамках своей школы
- [ ] **ORG-02**: Владелец может просматривать, редактировать и удалять роли своей школы
- [ ] **ORG-03**: Владелец может назначить набор permissions на роль (permission = строка-идентификатор CQRS-команды)
- [ ] **ORG-04**: Сервисы автоматически регистрируют свои permission-строки в каталоге при старте
- [ ] **ORG-05**: Владелец может назначить роль пользователю в рамках своей школы
- [ ] **ORG-06**: Владелец может отозвать роль у пользователя
...
- [ ] **ORG-10**: EF Core global query filter по schoolId во всех агрегатах (tenant isolation)
