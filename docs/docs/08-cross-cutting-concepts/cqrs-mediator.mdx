...
sidebar_position: 12
title: CQRS & Mediator
description: CQRS pattern implementation with MediatR for command query separation and pipeline behaviors
keywords: [CQRS, MediatR, command query separation, pipeline behaviors, validation, logging, handlers]
last_update:
  author: Nhan Nguyen
---
...
The Edvantix application implements Command Query Responsibility Segregation (CQRS) pattern using MediatR, providing clear separation between read and write operations with comprehensive pipeline behaviors for cross-cutting concerns.
