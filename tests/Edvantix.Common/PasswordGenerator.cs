...
internal static class PasswordGenerator
{
    ...
    ///     Using minLower, minUpper, minNumeric, and minSpecial also lowers the number of possible outputs and thus the bits
    ///     of entropy.
    ...
    )
    {
        if (!enabled && minValue > 0)
        {
            throw new ArgumentException(
                $"'{minValueParamName}' must be 0 if '{enabledParamName}' is disabled."
            );
        }
    }

    private static void GenerateRequiredValues(
    ...
    )
    {
        ...
        RandomNumberGenerator.GetItems(choices, destination[..minValues]);
        destination = destination[minValues..];
    }

    private static string GetChoices(bool lower, bool upper, bool numeric, bool special) =>
        (lower, upper, numeric, special) switch
        {
            (true, true, true, true) => LowerCaseChars
                + UpperCaseChars
                + NumericChars
                + SpecialChars,
            ...
            (false, false, false, false) => throw new ArgumentException(
