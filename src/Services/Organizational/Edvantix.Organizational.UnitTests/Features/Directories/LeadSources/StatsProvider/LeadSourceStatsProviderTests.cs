namespace Edvantix.Organizational.UnitTests.Features.Directories.LeadSources.StatsProvider;

public sealed class LeadSourceStatsProviderTests
{
    private readonly Mock<ILeadSourceRepository> _repoMock = new();
    private readonly Guid _orgId = Guid.CreateVersion7();
    private readonly LeadSourceStatsProvider _provider;

    public LeadSourceStatsProviderTests()
    {
        _provider = new LeadSourceStatsProvider(_repoMock.Object);
    }
    ...
    public void GivenProvider_WhenCheckingDescriptor_ThenShouldMatchCatalog()
    {
        ...
        descriptor.Code.ShouldBe(DirectoryCatalog.Sources);
        descriptor.Badge.ShouldBeNull();
    }

    [Test]
    public async Task GivenOrganizationWithSources_WhenGettingStats_ThenShouldReturnCorrectCounts()
    {
    ...
    [Test]
    public async Task GivenOrganizationWithNoSources_WhenGettingStats_ThenShouldReturnZeroCounts()
    {
        _repoMock
            .Setup(r =>
                r.CountAsync(It.IsAny<ISpecification<LeadSource>>(), It.IsAny<CancellationToken>())
