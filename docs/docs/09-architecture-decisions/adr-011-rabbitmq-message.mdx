...
keywords: [ADR-011, RabbitMQ, message broker, asynchronous messaging, MassTransit, event bus, messaging]
last_update:
  author: Nhan Nguyen
---
...
Edvantix's event-driven microservices architecture requires reliable asynchronous communication between services to maintain loose coupling and ensure system resilience. The messaging requirements include:
...
Adopt **RabbitMQ** as the primary message broker, integrated with **MassTransit** for .NET-based message handling, to provide reliable, scalable, and feature-rich asynchronous communication between Edvantix's microservices.
...
Edvantix uses a simplified event-driven architecture with RabbitMQ and MassTransit:
...
public class RabbitMQTopologyConfiguration
{
    public static void ConfigureTopology(IBusRegistrationConfigurator configurator)
    {
        ...
        {
            cfg.Host("rabbitmq-cluster", "/Edvantix", h =>
            {
                h.Username("Edvantix");
            ...
            cfg.ReceiveEndpoint("Edvantix.failed", e =>
            {
                e.ConfigureDeadLetterExchange("Edvantix.events.failed");
